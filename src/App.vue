  ...
  <main class="app" :data-night="isNight">
    ...
    <div class="sky">
      ...
      <button class="orb" @click="toggleTheme" aria-label="Toggle day/night" :title="isNight ? '아침으로 전환' : '밤으로 전환'">
        ...
        <svg v-else viewBox="0 0 120 120" class="sun">
          ...
          <g stroke="#ffad33" stroke-width="5" stroke-linecap="round">
            <line v-for="(r,i) in 12" :key="i" x1="60" y1="8" x2="60" y2="0" :transform="`rotate(${i*30} 60 60)`"/>
          </g>
        </svg>
      </button>
      ...
      <ul class="lanterns" aria-hidden="true">
        <li v-for="(l, i) in lanterns" :key="i" :style="lanternStyle(i)"></li>
      </ul>
    </div>
...
// subtle auto-rotate wishes
onMounted(() => {
  const t = setInterval(shuffleWish, 6000);
  // optional: return () => clearInterval(t) if you mount/unmount App
});

// lantern positions / speeds
const lanterns = Array.from({ length: 8 }, () => 0);
function lanternStyle(i: number) {
  const left = (i * 12 + 5) % 100; // spread across width
  const delay = (i % 5) * 1.2;
  const dur = 8 + (i % 4) * 2;
...
.star {
  position: absolute;
  width: 2px; height: 2px; background: #fff8d6;
  top: calc(var(--t, 0) * 1%); left: calc(var(--l, 0) * 1%);
  border-radius: 50%;
  opacity: 0.9;
...
.stars .star { --t: calc(100 * var(--rand, 0)); --l: calc(100 * var(--rand2, 0)); }
.stars .star::before { content: ''; }

/* Create pseudo-random positions without JS by using nth-child */
.star:nth-child(n) { top: calc((var(--i, 1) * 7) % 100 * 1%); left: calc((var(--i, 1) * 13) % 100 * 1%); }
.star:nth-child(1) { --i: 1 } .star:nth-child(2) { --i: 2 } .star:nth-child(3) { --i: 3 }
.star:nth-child(4) { --i: 4 } .star:nth-child(5) { --i: 5 } .star:nth-child(6) { --i: 6 }
.star:nth-child(7) { --i: 7 } .star:nth-child(8) { --i: 8 } .star:nth-child(9) { --i: 9 }
/* (80 items; we won't write all, CSS will wrap reasonably for visuals) */
...
.lanterns {
  ...
  position: absolute; inset-inline: 0; bottom: -20vh;
}
.lanterns li {
  ...
  box-shadow:
      ...
      0 0 24px rgba(255, 160, 64, 0.45);
  animation: floatUp linear infinite;
}
.lanterns li::before, .lanterns li::after {
  content: '';
  position: absolute; left: 50%; transform: translateX(-50%);
}
.lanterns li::before {
  top: -10px; width: 2px; height: 14px; background: rgba(255,255,255,0.5);
}
.lanterns li::after {
  bottom: -10px; width: 6px; height: 6px; background: #ffbe76; border-radius: 50%;
  box-shadow: 0 0 10px rgba(255,190,118,0.8);
}
@keyframes floatUp {
  from { transform: translateY(0) var(--extra, scale(1)); opacity: 0.95; }
  80% { opacity: 1; }
  to { transform: translateY(-120vh) var(--extra, scale(1)); opacity: 0; }
}
...
.wish {
  ...
  border-radius: 14px;
  background: linear-gradient(180deg, rgba(255,247,224,0.7), rgba(255,255,255,0.7));
...
@media (min-width: 760px) {
  .grid { grid-template-columns: 1.1fr 1fr 1.2fr; }
}
.panel {
  background: #ffffff;
...
.c-white { background:#e5e7eb; }
.c-black { background:#111827; }
...
.app[data-night="false"] .lanterns li { opacity: 0.75; box-shadow: 0 6px 12px rgba(0,0,0,0.1), 0 0 0 rgba(255,160,64,0); }
